...
const HealthPage = () => {
  ...
  useEffect(() => {
    const getHealth = async () => {
      ...
      const nodeJs = await apiGet(`${NODE_JS_SERVICE_API}/health`) // Adjust the endpoint as needed
        .then((r) => r.data)
        .catch(() => {
      ...
      const ruby = await apiGet(`${RUBY_SERVICE_API}/health`) // Adjust the endpoint as needed
        .then((r) => r.data)
        .catch(() => {
          setError('Failed to fetch health status');
          setLoading(false);
